...
% Optimization Loop
for i=1:T
    NoUpataGbestT(i)=0;%连续几代未更新gbest^t
end

HaveKnowledgeTranfer=0;%%已经执行知识迁移次数

for iter = 1:numIterations
    IsKnowledgeTranfer=rand()<=rmp; %%该代是否进行知识迁移
    if IsKnowledgeTranfer
        HaveKnowledgeTranfer=HaveKnowledgeTranfer+1;
    end
    w=0.9-0.5*(iter/numIterations);  %%更新权重系数
    fprintf('第 %d 次迭代\n', iter);
    for t = 1:T
        %每个任务
        ...
        for i = 1:numParticles
            ...
            r1 = rand();
            r2 = rand();
            if (IsKnowledgeTranfer && iter~=1)    %up particle base on gbest_new
                % 锦标赛选择gbest^m
                ...
                particles{t}(i).velocity = w * particles{t}(i).velocity + ...
                    ...
                    c2 * r2 .* (gbest_new - particles{t}(i).position);
            else % else no KnowledgeTranfer up base on gbest 
                particles{t}(i).velocity = w * particles{t}(i).velocity + ...
                    c1 * r1 .* (particles{t}(i).pbest - particles{t}(i).position) + ...
                    c2 * r2 .* (globalBests{t}.position - particles{t}(i).position);
            end
                newPosition = currentPosition + particles{t}(i).velocity; % 预更新粒子位置
       ...
       NoUpataGbestT(t)=NoUpataGbestT(t)+1;
    end %end this task
...
for t = 1:T
    ...
    % 该特征子集的分类正确率
    accuracy = evaluateFeatureSubset(selectedFeatures, X_train, Y_train, X_test, Y_test, 5);
